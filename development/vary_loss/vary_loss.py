...
new_model.compile(loss='huber', optimizer=optimizer, metrics=["MeanSquaredError"])

stop_early = tf.keras.callbacks.EarlyStopping(patience=20, restore_best_weights=True)
history = new_model.fit(X_train, y_train, epochs=1000, validation_data=(X_val, y_val), callbacks=[stop_early], verbose=1)

outdir = 'chains/'
outroot = f"cobaya-emulated-256-32-z0-M0-log"
only_minimize = False
...
g.triangle_plot(samples, params, filled=True, legend_labels=[f'ANN','Full MCMC (ngrid = 32)'])
...
plt.tight_layout()
plt.savefig(f'mcmc.png',dpi=300,bbox_inches='tight')  